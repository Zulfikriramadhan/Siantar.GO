...
<body class="bg-slate-50 font-sans min-h-screen text-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($drivers as $driver)
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                ...
                                <td class="px-6 py-4 font-bold text-emerald-700">🟢 {{ $driver->no_whatsapp }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-0.5 text-[10px] bg-slate-100 font-bold rounded uppercase">{{ $driver->jenis_kendaraan }}</span>
                                    <div class="text-xs text-slate-800 font-bold mt-1">{{ $driver->nomor_plat }}</div>
                                </td>
                                <td class="px-6 py-4">
    ...
    <script>
        ...
        function openApprovalModal(id, name) {
            ...
            // Set rute action form sesuai ID driver yang diklik
            document.getElementById('formApprove').action = `/admin/verifikasi/${id}`;
            modal.classList.remove('hidden');

            // Atur ulang ukuran kanvas saat modal muncul terbuka
